...
import static com.amazon.opendistroforelasticsearch.sql.common.utils.StringUtils.stripLeading;
import static com.amazon.opendistroforelasticsearch.sql.common.utils.StringUtils.stripTrailing;
import static com.amazon.opendistroforelasticsearch.sql.data.model.ExprValueUtils.*;
...
@Ignore("Try to fix lombok issues here")
public class TextFunctionTest extends ExpressionTestBase {
    ...
    static class StringPatternPair {
        ...
        public StringPatternPair(String str, String patt) {
            this.str = str;
            this.patt = patt;
        }
    ...
    static class SubstringInfo {
        ...
        public SubstringInfo(String expr, Integer start, Integer len, String res) {
            this.expr = expr;
            this.start = start;
            this.len = len;
            this.res = res;
    ...
    void ltrimString(String str) {
        ...
        assertEquals(stripLeading(str.getBytes()), eval(expression).stringValue());
    }

    void rtrimString(String str) {
        FunctionExpression expression = dsl.rtrim(DSL.literal(str));
        assertEquals(STRING, expression.type());
        assertEquals(stripTrailing(str.getBytes()), eval(expression).stringValue());
    }
    ...
    void testLengthString(String str) {
        ...
        assertEquals(str.getBytes().length, eval(expression).integerValue().intValue());
    }
    ...
    void testStcmpString(StringPatternPair stringPatternPair) {
        ...
        assertEquals(INTEGER, expression.type());
        assertEquals(stringPatternPair.strCmpTest(), eval(expression).integerValue().intValue());
    }
