...
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_ACCOUNT;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_DOG;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_LOCATION;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_NESTED_TYPE;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_PEOPLE;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_PHRASE;
...
public class ExplainIT extends SQLIntegTestCase {
  ...
  public void searchSanity() throws IOException {
    ...
    String result =
        explainQuery(String.format("SELECT * FROM %s WHERE firstname LIKE 'A%%' AND age > 20 " +
  ...
  public void aggregationQuery() throws IOException {
    ...
    String result = explainQuery(
        String.format("SELECT address, CASE WHEN gender='0' then 'aaa' else 'bbb'end a2345," +
  ...
  public void betweenScriptValue() throws IOException {
    ...
    String result =
        explainQuery(String.format("SELECT  case when balance between 100 and 200 then 'aaa' " +
  ...
  public void deleteSanity() throws IOException {
    ...
    String result =
        explainQuery(String.format("DELETE FROM %s WHERE firstname LIKE 'A%%' AND age > 20",
            TEST_INDEX_ACCOUNT));
    Assert
        .assertThat(result.replaceAll("\\s+", ""), equalTo(expectedOutput.replaceAll("\\s+", "")));
  }
  ...
  public void orderByOnNestedFieldTest() throws Exception {
    ...
    String result =
        explainQuery(String.format("SELECT * FROM %s ORDER BY NESTED('message.info','message')",
  ...
  public void multiMatchQuery() throws IOException {
    ...
    String result =
        explainQuery(String.format("SELECT * FROM %s WHERE multimatch('query'='this is a test'," +
  ...
  public void termsIncludeExcludeExplainTest() throws IOException {
    ...
    String result =
        explainQuery(queryPrefix + " terms('field'='correspond_brand_name','size'='10'," +
  ...
  public void explainNLJoin() throws IOException {
    ...
    String query = "SELECT /*! USE_NL*/ a.firstname ,a.lastname , a.gender ,d.dog_name  FROM " +
        TEST_INDEX_PEOPLE + "/people a JOIN " + TEST_INDEX_DOG +
