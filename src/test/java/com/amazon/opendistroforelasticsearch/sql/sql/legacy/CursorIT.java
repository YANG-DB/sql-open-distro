...
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestUtils.getResponseBody;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_ACCOUNT;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_DATE_TIME;
import static com.amazon.opendistroforelasticsearch.sql.sql.legacy.TestsConstants.TEST_INDEX_NESTED_SIMPLE;
...
public class CursorIT extends SQLIntegTestCase {
  ...
  public void testCursorSettings() throws IOException {
    ...
    assertThat(clusterSettings.query("/defaults/opendistro.sql.cursor.fetch_size"),
        equalTo("1000"));
    ...
    updateClusterSettings(
        new ClusterSetting(PERSISTENT, "opendistro.sql.cursor.keep_alive", "200s"));
    ...
    assertThat(clusterSettings.query("/transient/opendistro.sql.cursor.fetch_size"),
        equalTo("400"));
    assertThat(clusterSettings.query("/persistent/opendistro.sql.cursor.keep_alive"),
        equalTo("200s"));
  ...
  public void verifyWithAndWithoutPaginationResponse(String sqlQuery, String cursorQuery,
    ...
    verifySchema(withoutCursorResponse.optJSONArray(SCHEMA),
        withCursorResponse.optJSONArray(SCHEMA));
    verifyDataRows(withoutCursorResponse.optJSONArray(DATAROWS),
        withCursorResponse.optJSONArray(DATAROWS));
  }
  ...
  private void enableCursorClusterSetting() throws IOException {
    updateClusterSettings(
        new ClusterSetting("persistent", "opendistro.sql.cursor.enabled", "true"));
  }
